...
%                   % the rest and get a new point
    ...
    while (d > 0.001)
        ...
        % steps
        q_buff_rad(:,1) = linspace(q0_rad(2),q_rad(2),1);
        q_buff_rad(:,2) = linspace(q0_rad(3),q_rad(3),1);
        ...
        for i=1:1
            ...
            all_jointPos = fwkin_all(0, q_cur_deg(1), q_cur_deg(2));
    ...
    % Set v1 and v2 vectors to be the difference from the initial joint
    % angles to the two solution of inverse kinematics
%     v1 = q(2,:)-q0;
%     v2 = q(3,:)-q0;
    
%     v= q-q0;
    
    % Set q to be the closer solution (in the sense of G matrix) 
%     if( v1*G*v1' < v2*G*v2' )
%         q = q(2,:);
%     else
%         q = q(3,:);
...
%     % Divide the distance between current and desired joint angles into 100
%     % steps
%     q_(:,1) = linspace(q0(2),q(1,2),100);
%     q_(:,2) = linspace(q0(3),q(1,3),100);
% 
%     % Move the target point of the figure to x and y coordinates
%     set(point,'XData',x,'YData',y);
%     
