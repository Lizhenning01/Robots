...
try
    ...
    for i = 1:6
        sortObjects('duck', cam, camParam, pp, STATUS,GRIPPER,SET_POS);
        close all;
    end
...
path = ikin_diff_traj([ball(1),ball(2),-45],ikin(ball(1),ball(2),20));
%     pause(0.1);
...
while noObject <= 2 
    ...
    if (num_ball < 1)      
        ...
        noObject = 5;        
        objectFound =1;
         %move to location above the ball
        
        cur_ball = [ball_pos(1,1),ball_pos(1,2)];
        moveToBall(cur_ball,pp,STATUS,GRIPPER,SET_POS);
