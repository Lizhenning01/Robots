...
//float myHomePosition[3] = {-787,-1039,2196}; //manual calibration

float myHomePosition[3] = {0,0,0};
...
int main() {
	...
	//myHomePosition = getHomePosition();
#ifdef DEBUG
	...
	Servo* gripper = new Servo(GRIPPER, 5);
#endif
	...
	for (int i = 0; i < DOFs; i++) // for each joint,
		...
		pid[i]->ZeroPID();
#else            // else, use the values in myHomePosition 
		pid[i]->pidReset(pid[i]->GetPIDPosition() - myHomePosition[i]);
#endif
	...
	coms.attach(new CalibrationServer(pid, DOFs));
	coms.attach(new ServoServer(gripper));
...
// print encoder values for each joint
			...
			//printf("\r\nhome Value = %f , %f , %f", myHomePosition[0],
					//myHomePosition[1],myHomePosition[2]);
